...
const i18n = {
  ar: {
    ...
    ask_keyword: '🔎 اكتب اسم السلعة اللي تبحث عنها:\n\n✅ اكتب بس الكلمات الأساسية المميزة (الماركة + الموديل مثلاً)، بدون جمل أو كلمات زايدة — النظام يدوّر على كل كلمة تكتبها بأي مكان بالإعلان، فكل كلمة إضافية لازم توجد حرفياً وإلا ما يطابق.\n\nمثال صح ✅: نيسان باترول 2025\nمثال غلط ❌: أبحث عن سيارة نيسان باترول موديل 2025 نظيفة\n(هذا لأن كلمات زي "أبحث" و"نظيفة" غالباً ما تكون موجودة بنص الإعلان، فتمنع المطابقة رغم إن السيارة نفسها متوفرة)\n\n💡 لو السلعة لها أكثر من طريقة كتابة شائعة (خصوصاً أسماء معرّبة)، اكتبها كلها مفصولة بفاصلة:\nمثال: نيسان باترول 2025, نيسان بترول 2025',
  ...
  en: {
    ...
    ask_keyword: '🔎 Enter the item you\'re looking for:\n\n✅ Use only the key distinguishing words (brand + model, for example) — no full sentences or extra words. The system looks for every word you type anywhere in the ad, so each extra word must literally appear too, or it won\'t match.\n\nGood ✅: Nissan Patrol 2025\nBad ❌: Looking for a clean Nissan Patrol model 2025\n(words like "Looking" and "clean" usually aren\'t in the actual ad text, so they\'d block a real match even if the car is genuinely listed)\n\n💡 If the item is commonly written more than one way (especially transliterated names), list them all separated by a comma:\ne.g.: Nissan Patrol 2025, Nissan Baterol 2025',
...
// مطابقة "كل الكلمات موجودة بأي ترتيب" بدل "العبارة الكاملة متلاصقة". مشكلة
// حقيقية واجهناها: بحث "نيسان باترول 2025" ما طابق إعلانات حقيقية فيها
// السيارة بالضبط لأن عنوانها الفعلي "نيسان باترول اس اي تيتانيوم 2025" —
// كلمة الفئة (تيتانيوم/بلاتينيوم...) تتوسط بين الموديل والسنة دائماً تقريباً،
// فمطابقة العبارة الكاملة كوحدة واحدة تفشل حرفياً مع كل إعلان سيارة تقريباً.
// كل كلمة لحالها لسا محمية بنفس فحص حدود الأحرف العربية (يمنع "ثلاثي" من
// مطابقة "ثلاثية") — بس الشرط الحين إنها موجودة بالنص، مو ملاصقة للكلمة التالية.
function buildKeywordMatcher(keyword) {
  const wordPatterns = keyword
    .split(/\s+/)
    .filter(Boolean)
    .map((word) => {
  ...
  return { test: (text) => wordPatterns.every((pattern) => pattern.test(text)) };
}
